...
export psi_tab, M_arr, psi_calc, psi_eff, psi_th, psi_tot_tab
...
export dr, dv, dlogr, dlogv, int_steps, tol, maxiter, epsilon
...
# --- Effective potential for orbit finding ---
function psi_eff(r,L)

    if r < r_tab[1] # r < r1 -> analytic  
        return -G*M_bh/r + psi_tab[1] + (L^2)/(2*r^2)

    elseif r > r_tab[N] # r > rN keplerian
        return -(G*M_tot)/r + (L^2)/(2*r^2)
...
function read_file(filename)
    ...
    global coulomb_log = M_bh / mean(m_tab) # log(0.11*N)
    global m_test = mean(m_tab) # 1/N
    global m_field = m_test
    global M_tot = sum(m_tab) + M_bh

    # find potential via Henon
    global psi_tab, M_arr = find_psi_arrays(r_tab,m_tab)
    global psi_calc = psi_exact(r_tab,psi_tab,M_arr)

    # total potential table (including BH), effective potential for orbit finding 
    global psi_tot_tab = psi_tab[1:end-1] - G * M_bh ./ r_tab
