...
## Compute velocity diffusion coefficients for given (r,E,L) ##
function find_coef_lin(r,E,L,m_obj) # Linear sampling

    # Determine object's total velocity
    vel = sqrt(find_vr(r,E,L) ^ 2 + find_vt(r,E,L) ^ 2)

    # Coulomb logarithm
    cou_log =  log(M_bh / mean(dat.m)) #log(0.11*dat.N)
    ...
    # Calculate integrals via midpoint
...
function find_coef_log(r,E,L,m_obj) # Logarithmic sampling
    ...
    # Determine object velocity; Convert (r,v) -> (logr,logv)
    vel = sqrt(find_vr(r,E,L) ^ 2 + find_vt(r,E,L) ^ 2)
