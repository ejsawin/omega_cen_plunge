...
function avg_coef_int(tt,EE,LL,ii,rp,ra)
    ...
    rrr=r_hat*sin(tt)+t_hat # Eq 2.17
    vr_tilde=v_r(rrr,EE,LL)/(r_hat*cos(tt)) # Eq 2.20

    return (loc_coef(rrr,EE,LL)[ii])/vr_tilde
end

function avg_coef(EE_,LL_,indx) # Eq 2.55 - 2.59
    rp,ra=find_rp_ra(EE_,LL_)
    T=period(EE_,LL_)
