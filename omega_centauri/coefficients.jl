...
function vel_coef_trap(r,E,L)
    ...
    # Compute integrals via trapezoid
    calc1 = trapezoid(x->int1(x),log(dat.vmin),log_vel,n_steps)
    calc2 = trapezoid(x->int2(x),log(dat.vmin),log_vel,n_steps)
    calc3 = trapezoid(x->int3(x),log(dat.vmin),log_vel,n_steps)
    calc4 = trapezoid(x->int4(x),log_vel,log(dat.vmax),n_steps)
